...
#define RCCHECK(fn)                                                            \
  {                                                                            \
    ...
    if ((temp_rc != RCL_RET_OK)) {                                             \
      char message[128];                                                       \
      sprintf(message, "Error on line %d with status %d. Aborting.\n",         \
...
#define RCSOFTCHECK(fn)                                                        \
  {                                                                            \
    rcl_ret_t temp_rc = fn;                                                    \
    if ((temp_rc !=                                                            \
         RCL_RET_OK)) { /*char message[128];sprintf(message, "Error on line %d \
                           with status %d. Continuing.\n", __LINE__,           \
                           (int)temp_rc);M5.lcd.println(message);*/            \
    }                                                                          \
  }
...
// Stops the robot if no vectors are recieved after VECTOR_STOP_TIME timer
// updates.
...
void drawMarker(u_int64_t data, uint32_t background) {
  ...
  M5.lcd.fillRect(WIDTH - size - side_inset, 0, size + side_inset, HEIGHT,
                  background);
...
void configure_robot() {
  ...
  RCCHECK(rclc_publisher_init_default(
      &register_publisher, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "/setup/register"));
  ...
  RCCHECK(rclc_publisher_init_default(
      ...
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Bool), heartbeat_topic_name));
  ...
  RCCHECK(rclc_publisher_init_default(
      &pose_publisher, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Pose), pose_topic_name));
  ...
  RCCHECK(rclc_subscription_init_default(
      &marker_subscriber, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int64), marker_topic_name));
  handle_count++;
  ...
  RCCHECK(rclc_subscription_init_default(
      &colour_subscriber, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, ColorRGBA),
      colour_topic_name));
  handle_count++;

  // Sets up the executor
  RCCHECK(rclc_executor_init(&executor, &support.context, handle_count,
                             &allocator));

  // Adds the vector subscription to the executor
  RCCHECK(rclc_executor_add_subscription(&executor, &vector_subscriber,
                                         &vector_msg, &vector_callback,
                                         ON_NEW_DATA));

  // Adds the pose subscription to the executor
  RCCHECK(rclc_executor_add_subscription(&executor, &camera_pose_subscriber,
                                         &camera_pose_msg,
                                         &camera_pose_callback, ON_NEW_DATA));

  // Adds the vector subscription to the executor
  RCCHECK(rclc_executor_add_subscription(&executor, &uncertainty_subscriber,
                                         &uncertainty_msg,
                                         &uncertainty_callback, ON_NEW_DATA));

  // Adds the marker subscription to the executor
  RCCHECK(rclc_executor_add_subscription(&executor, &marker_subscriber,
                                         &marker_msg, &marker_callback,
                                         ON_NEW_DATA));

  // Adds the colour subscription to the executor
  RCCHECK(rclc_executor_add_subscription(&executor, &colour_subscriber,
                                         &colour_msg, &colour_callback,
                                         ON_NEW_DATA));
...
void setup() {
  ...
  set_microros_wifi_transports("TP-Link_102C", "35811152", "192.168.0.230",
                               8888);
  ...
  // Create a ROS node
  RCCHECK(rclc_node_init_default(&setup_node, "temporary_robot_setup_node", "",
                                 &support));

  RCCHECK(rclc_publisher_init_default(
      &register_publisher, &setup_node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "/setup/register"));

  RCCHECK(rclc_subscription_init_default(
      &id_subscription, &setup_node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "/setup/ids"));

  // Creates an executor to handle the subscriptions
  RCCHECK(rclc_executor_init(&setup_executor, &support.context, 1, &allocator));

  RCCHECK(rclc_executor_add_subscription(&setup_executor, &id_subscription,
                                         &id_msg, &id_callback, ON_NEW_DATA));
...
bool timer_callback(void* args) {
  ...
  // Send a zero vector to stop the 3pi if no vectors have been recieved
  // recently
...
void loop() {
  // Checks how long the timer has been running, and calls timer callback if
  // needed
  timer.tick();

  if (!configured) {
    // Checks for messages from the setup subscriptions if the robot isnt
    // configured
